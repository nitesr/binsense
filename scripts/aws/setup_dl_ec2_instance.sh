...
function create_sg () {
    ...
    if [ $? -eq 0 ]; then
        ...
        aws ec2 authorize-security-group-ingress --group-name $SG_NAME --ip-permissions "[ {  \"IpProtocol\":  \"tcp\",  \"FromPort\": 22,  \"ToPort\": 22,  \"IpRanges\": [ {  \"CidrIp\":  \"0.0.0.0/0\" } ] } ]" --output yaml --profile $PROFILE
